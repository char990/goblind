...
void Refresh_TZ()
{
    ...
    struct stat st_buf;
    int r = stat (_goblin_user, &st_buf);
    if(r!=0)
    ...
    // goblin_user was not changed, reload TZ
    goblin_user_time=st_buf.st_mtime;
    FILE *fd = popen("uci get goblin_user.user_cfg.TZ", "r");
    int city_i=4;
    int cnt=0;
    char city[10];
    memset(city,0,10);
...
char * Get_current_time()
{
    ...
    if(t!=st)
    {
        ...
        int len = snprintf(time_buf, 31, "<%d/%02d/%04d %d:%02d:%02d>",
